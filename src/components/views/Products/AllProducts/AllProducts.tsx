...
import categoryData from "@/data/products/categoryProduct.json";
...
export default function AllProducts() {
  ...
  const filteredProducts =
    activeCategory === "all"
      ...
      ? productsData.products.filter((product) => product.category === "import")
      : productsData.products.filter(
          (product) => product.category === "export"
        );
      ...
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-[var(--color-neutral-gray)] max-w-3xl mx-auto">
            Find the best logistics solutions for your import and export
            business needs
          </p>
        </div>

        {/* Category Filter dengan Border */}
        <div className="flex flex-wrap justify-center gap-4 mb-16">
          {/* Button Semua Produk */}
          <button
            onClick={() => setActiveCategory("all")}
            className={`px-8 py-4 rounded-2xl font-semibold transition-all duration-300 border-2 ${
              activeCategory === "all"
                ? "bg-gradient-to-r from-[var(--color-primary)] to-[var(--color-primary-light)] text-white border-[var(--color-primary)] shadow-xl transform scale-105"
                : "bg-white text-[var(--color-neutral-gray)] border-[var(--color-neutral-light-gray)] hover:border-[var(--color-primary)] hover:text-[var(--color-primary)] hover:shadow-lg hover:scale-105"
            }`}
          >
            <div className="flex items-center space-x-2">All Products</div>
          </button>

          {/* Button dari data JSON */}
          {categoryData.categories.map((category) => (
            <button
              key={category.id}
              onClick={() =>
                setActiveCategory(
                  category.name.toLowerCase() as "import" | "export"
                )
              }
              className={`px-8 py-4 rounded-2xl font-semibold transition-all duration-300 border-2 ${
            ...
            >
              <div className="flex items-center space-x-2">{category.name}</div>
            </button>
          ))}
        </div>
              ...
              <Link href={`/products/${product.slug}`} className="block">
                <div className="relative h-56 overflow-hidden">
                  ...
                  <div className="absolute top-4 left-4">
                    ...
                    >
                      {product.category === "import" ? "IMPORT" : "EXPORT"}
                    </span>
                  </div>
